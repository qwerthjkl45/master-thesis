...
def hw_flatten(x):
    ...
    return tf.reshape(x, shape=[-1, x_shape[1]*x_shape[2], x_shape[-1]])


def attention(x, out_dim, scope='attention', reuse=False):
    return x 
    with tf.variable_scope(scope, reuse=reuse):
        ...
        x_shape = x.get_shape().as_list()
        o = tf.reshape(o, shape=[-1, x_shape[1], x_shape[2], x_shape[3]])  # [bs, h, w, C]
        x = gamma * o + x
    return x
