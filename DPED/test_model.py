...
from models_with_sa import unet
import utils
import os
import sys

# process command arguments
phone, dped_dir, test_subset, iteration, resolution, use_gpu, model_dir = utils.process_test_model_args(sys.argv)
...
with tf.Session(config=config) as sess:
    ...
    if phone.endswith("_orig"):
        ...
        num_saved_models = int(len([f for f in os.listdir(model_dir) if f.startswith(phone + "_iteration")]) / 2)

        if iteration == "all":
            iteration = np.arange(1, num_saved_models_v1) * 1000
        else:
        ...
        for i in iteration:
            # load pre-trained model
            saver = tf.train.Saver()
            saver.restore(sess, model_dir + phone + "_iteration_" + str(i) + ".ckpt")
            for photo in test_photos:
                ...
                #image = np.float16(misc.imresize(misc.imread(test_dir + photo), res_sizes[phone])) / 255
                image = np.float16((misc.imread(test_dir + photo))) / 255
                image_crop = image
                #image_crop = utils.extract_crop(image, resolution, phone, res_sizes)
                image_crop_2d = np.reshape(image_crop, [1, IMAGE_SIZE])
